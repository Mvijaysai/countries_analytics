...
interface Filters {
  region: string;
  searchTerm: string;
  sortOrder: "asc" | "desc" | "default" | string ;
}

export const useCountryData = () => {
  const [originalData, setOriginalData] = useState<CountryDetails[]>([]);
  const [filteredData, setFilteredData] = useState<CountryDetails[]>([]);
  ...
  useEffect(() => {
    const fetchCountryData = async () => {
      try {
        ...
        const uniqueRegions : Array<string> = Array.from(new Set(data.map((country:CountryDetails) => country.region)));
